...
router.post('/saveInfo', async (req, res, next) => {
  let {short_link, phid} = req.body
  try {
    //先去数据库里查短链接是否存在
    let exist = await query('select * from fg_shortlink where short_link = ?', [short_link])
    if (exist.length !== 0) {
      return res.json({
        success: false,
        errorMsg: '短链接已经存在啦，请重新换一个吧',
        code: 500
      })
    }
    let time = new Date().valueOf()
    let sql = await query('update fg_shortlink set short_link = ?,updated_at = ?  where phid = ?', [short_link, time, phid])
